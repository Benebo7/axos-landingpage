...
export default function CardFlip({
  ...
  // Only generate random values on client after mount to avoid hydration mismatch
  const [animationBlocks, setAnimationBlocks] = useState<Array<{width: string, marginLeft: string, delay: string}>>([]);
  ...
  // Auto-flip quando autoFlip mudar para true
  useEffect(() => {
    if (autoFlip) {
      const timer = setTimeout(() => {
        setIsFlipped(true);
      }, flipDelay); // Delay customizável após aparecer na tela
  ...
  return (
    <div
      ...
      className="group relative h-[300px] sm:h-[320px] md:h-[340px] w-full max-w-[260px] sm:max-w-[280px] [perspective:2000px]"
      onMouseEnter={() => setIsFlipped(true)}
      onMouseLeave={() => {
        // Só volta ao estado anterior se não estiver em autoFlip
        if (!autoFlip) {
          setIsFlipped(false);
        }
      }}
      ...
      >
        ...
        <div
          className={cn(
            ...
            'transition-all duration-700',
            'group-hover:shadow-xl group-hover:shadow-black/20 dark:group-hover:shadow-black/60',
            'group-hover:border-zinc-300/60 dark:group-hover:border-white/20',
            isFlipped ? 'opacity-0' : 'opacity-100',
          )}
        >
          {/* Background gradient effect */}
          <div className="from-white/10 absolute inset-0 bg-gradient-to-br via-transparent to-white/5 dark:from-white/5 dark:via-transparent dark:to-white/0" />

          {/* Animated code blocks */}
          <div className="absolute inset-0 flex items-center justify-center pt-16">
            <div className="relative flex h-[90px] w-[180px] flex-col items-center justify-center gap-2">
              {/* Code blocks animation - only render after mount */}
              {isMounted && animationBlocks.map((block, i) => (
                <div
                  key={i}
                  className={cn(
        ...
        >
          {/* Background gradient */}
          <div className="from-white/10 absolute inset-0 rounded-2xl bg-gradient-to-br via-transparent to-white/5 dark:from-white/5 dark:via-transparent dark:to-white/0" />
